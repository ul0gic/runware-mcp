...
- Patched `fast-uri` (host confusion via skipped IDN canonicalization), `qs`
  (array-limit bypass, `isBuffer` DoS), `hono` (`toSSG` path escape, `parseBody`
  nesting exhaustion, fragment query parsing), and `@humanfs/node` (symlink
  copy) (#39).
- Moved to `vitest` 5 for the `@vitest/mocker` path traversal advisory, which
  had no fix on the 4.x line (#39).
