import { describe, it, expect, vi, beforeEach } from 'vitest';

vi.mock('../../../src/shared/rate-limiter.js', () => ({
  defaultRateLimiter: {
    waitForToken: vi.fn().mockResolvedValue(undefined),
  },
}));

vi.mock('../../../src/shared/config.js', () => ({
  config: {
    RUNWARE_API_KEY: 'test-api-key-that-is-at-least-32-characters-long',
    REQUEST_TIMEOUT_MS: 60000,
    POLL_MAX_ATTEMPTS: 150,
    MAX_FILE_SIZE_MB: 50,
    ALLOWED_FILE_ROOTS: [],
    LOG_LEVEL: 'error',
    NODE_ENV: 'test',
    RATE_LIMIT_MAX_TOKENS: 10,
    RATE_LIMIT_REFILL_RATE: 1,
    WATCH_FOLDERS: [],
    WATCH_DEBOUNCE_MS: 500,
  },
  API_BASE_URL: 'https://api.runware.ai/v1',
}));

import {
  imageUpscale,
  imageUpscaleToolDefinition,
  imageUpscaleInputSchema,
} from '../../../src/tools/image-upscale/index.js';
import type { RunwareClient } from '../../../src/integrations/runware/client.js';

function createMockClient(): RunwareClient {
  return {
    request: vi.fn(),
    requestSingle: vi.fn(),
    generateTaskUUID: vi.fn().mockReturnValue('mock-uuid'),
  } as unknown as RunwareClient;
}

describe('imageUpscale', () => {
  let mockClient: RunwareClient;

  beforeEach(() => {
    vi.clearAllMocks();
    mockClient = createMockClient();
  });

  describe('handler', () => {
    it('should return success with valid input', async () => {
      const mockResponse = {
        taskType: 'upscale',
        taskUUID: 'test-task-uuid',
        imageUUID: 'upscaled-image-uuid',
        imageURL: 'https://example.com/upscaled.png',
        cost: 0.005,
      };

      (mockClient.requestSingle as ReturnType<typeof vi.fn>).mockResolvedValue(mockResponse);

      const result = await imageUpscale(
        { inputImage: 'https://example.com/image.jpg', upscaleFactor: 2 },
        mockClient,
      );

      expect(result.status).toBe('success');
      expect(result.message).toContain('upscaled 2x');

      const data = result.data as { imageUUID: string; imageURL: string };
      expect(data.imageUUID).toBe('upscaled-image-uuid');
      expect(data.imageURL).toBe('https://example.com/upscaled.png');
    });

    it('should handle 4x upscale', async () => {
      const mockResponse = {
        taskType: 'upscale',
        taskUUID: 'uuid',
        imageUUID: 'img-uuid',
        imageURL: 'https://example.com/upscaled.png',
        cost: 0.01,
      };

      (mockClient.requestSingle as ReturnType<typeof vi.fn>).mockResolvedValue(mockResponse);

      const result = await imageUpscale(
        { inputImage: 'https://example.com/image.jpg', upscaleFactor: 4 },
        mockClient,
      );

      expect(result.status).toBe('success');
      expect(result.message).toContain('upscaled 4x');
    });

    it('should return error when API fails', async () => {
      (mockClient.requestSingle as ReturnType<typeof vi.fn>).mockRejectedValue(
        new Error('Upscale failed'),
      );

      const result = await imageUpscale(
        { inputImage: 'https://example.com/image.jpg' },
        mockClient,
      );

      expect(result.status).toBe('error');
      expect(result.message).toContain('Upscale failed');
    });

    it('should propagate cost from response', async () => {
      const mockResponse = {
        taskType: 'upscale',
        taskUUID: 'uuid',
        imageUUID: 'img-uuid',
        imageURL: 'https://example.com/upscaled.png',
        cost: 0.007,
      };

      (mockClient.requestSingle as ReturnType<typeof vi.fn>).mockResolvedValue(mockResponse);

      const result = await imageUpscale(
        { inputImage: 'https://example.com/image.jpg' },
        mockClient,
      );

      expect(result.status).toBe('success');
      expect(result.cost).toBe(0.007);
    });
  });

  describe('schema', () => {
    it('should validate correct input with required fields', () => {
      const result = imageUpscaleInputSchema.safeParse({
        inputImage: 'https://example.com/image.jpg',
      });
      expect(result.success).toBe(true);
    });

    it('should apply default values', () => {
      const result = imageUpscaleInputSchema.safeParse({
        inputImage: 'https://example.com/image.jpg',
      });

      expect(result.success).toBe(true);
      if (result.success) {
        expect(result.data.upscaleFactor).toBe(2);
        expect(result.data.includeCost).toBe(true);
      }
    });

    it('should reject missing inputImage', () => {
      const result = imageUpscaleInputSchema.safeParse({});
      expect(result.success).toBe(false);
    });

    it('should accept optional model parameter', () => {
      const result = imageUpscaleInputSchema.safeParse({
        inputImage: 'https://example.com/image.jpg',
        upscaleFactor: 4,
        model: 'runware:101@2',
      });
      expect(result.success).toBe(true);
    });
  });

  describe('tool definition', () => {
    it('should have correct name', () => {
      expect(imageUpscaleToolDefinition.name).toBe('imageUpscale');
    });

    it('should require inputImage', () => {
      expect(imageUpscaleToolDefinition.inputSchema.required).toContain('inputImage');
    });

    it('should have a description', () => {
      expect(imageUpscaleToolDefinition.description).toBeTruthy();
    });
  });
});
